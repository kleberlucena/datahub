...
class Command(BaseCommand):
    ...
    def create_groups(self):
        groups_to_create = [
            "profile:georeference_basic", #user can list areas and categories
            "profile:georeference_advanced", #user can list/create/update areas and categories
            "profile:georeference_manager", #user can list/create//delete areas and categories
        ]
        ...
        with transaction.atomic():
            for group_name in groups_to_create:
                ...
                # Assign permissions to "profile:georeference_basic" group
                if group_name == "profile:georeference_basic":
                    permissions = []
                ...
                # Assign permissions to "profile:georeference_advanced" group
                if group_name == "profile:georeference_advanced":
                    permissions = []
                    for model in models_to_get:
                        content_type = ContentType.objects.get_for_model(model)
                        model_permissions = Permission.objects.filter(
                ...
                # Assign permissions to "profile:georeference_manager" group
                if group_name == "profile:georeference_manager":
                    permissions = []
                    for model in models_to_get:
                        content_type = ContentType.objects.get_for_model(model)
                        model_permissions = Permission.objects.filter(
