...
class AddFactListView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        has_city = Q()
        address_neighborhood = self.request.query_params.get(
            'address_neighborhood')
        has_neighborhood = Q()
        address_street = self.request.query_params.get('address_street')
        has_street = Q()
        address_complement = self.request.query_params.get(
            'address_complement')
        has_complement = Q()
        ...
        has_father = Q()
        document_birth_date = self.request.query_params.get(
            'document_birth_date')
        has_birth_date = Q()
        ...
        if address_neighborhood is not None:
            has_neighborhood = Q(
                addresses__neighborhood__unaccent__icontains=address_neighborhood)
        if address_street is not None:
            has_street = Q(
                addresses__street__unaccent__icontains=address_street)
        if address_complement is not None:
            has_complement = Q(
                addresses__complement__unaccent__icontains=address_complement)
        if address_reference is not None:
            has_reference = Q(
                addresses__reference__unaccent__icontains=address_reference)
        if address_zipcode is not None:
            has_zipcode = Q(addresses__zipcode__icontains=address_zipcode)
        if document_name is not None:
            has_name = Q(suspects__documents__name__unaccent__icontains=document_name) | Q(
                suspects__nicknames__label__unaccent__icontains=document_name)
        if document_mother is not None:
            has_mother = Q(
                suspects__documents__mother__unaccent__icontains=document_mother)
        if document_father is not None:
            has_father = Q(
                suspects__documents__father__unaccent__icontains=document_father)
        if document_birth_date is not None:
            has_birth_date = Q(
                suspects__documents__birth_date__icontains=document_birth_date)
        if document_number is not None:
            has_number = Q(
                suspects__documents__number__icontains=document_number)
        if nickname_label is not None:
            has_nickname = Q(suspects__nicknames__label__unaccent__icontains=nickname_label) | Q(
                suspects__documents__name__unaccent__icontains=nickname_label)
            ...
            try:
                ...
                if serializer.is_valid():
                    ...
                    for address in instance.addresses.all():
                        ...
                        address.save()
                        assign_perm("change_address",
                                    self.request.user, address)
                        assign_perm("delete_address",
                                    self.request.user, address)
                    ...
                    for image in instance.images.all():
                        ...
                        image.save()
                        assign_perm("change_factimage",
                                    self.request.user, image)
                        assign_perm("delete_factimage",
                                    self.request.user, image)
