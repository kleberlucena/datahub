...
def update_registers(person_renavam_cortex):
    try:
        ...
        logger.error('Error while update or create Registry vehicle_cortex - {}'.format(e))
...
def cortex_consult_vehicle_by_cpf(self, username, cpf):
    ...
    retorno = None
    try:
        vehicles_json = portalCortexService.get_vehicle_by_proprietario(cpf=cpf, username=username)
    except Exception as e:
        logger.error('Error while getting vehicles_cortex by CPF - {}'.format(e))
    try:
...
def add_arrendatario(arrendatario_json):
    try:
        if arrendatario_json["id"] == 0 or arrendatario_json["id"] == None:
            ...
            person = None
            try:
                person = PersonRenavamCortex.objects.get(id=arrendatario_json["id"])
...
def add_proprietario(proprietario_json):
    try:
        if proprietario_json["id"] == 0 or proprietario_json["id"] == None:
            ...
            person = None
            try:
                person = PersonRenavamCortex.objects.get(id=proprietario_json["id"])
            except PersonRenavamCortex.DoesNotExist:
                person = PersonRenavamCortex.objects.create(
...
def add_possuidor(possuidor_json):
    try:
        if possuidor_json["id"] == 0 or possuidor_json["id"] == None:
            ...
            person = None
            try:
                person = PersonRenavamCortex.objects.get(id=possuidor_json["id"])
            except PersonRenavamCortex.DoesNotExist:
