...
from apps.rpa.forms import AircraftSelectionForm, TypeOfBatteryForm
from apps.rpa.utils.createJsonByLastOperation import createJsonByLastOperation
from apps.rpa.utils.getTodayLatLonCoordinates import getTodaysCoordinates
from apps.rpa.utils.getOperationInCourse import getOperationInCourse

from apps.rpa.utils.rpaManagerDashboard import *
from apps.portal.models import *
from apps.rpa.models import *
from datetime import datetime

def home(request):
    return render(request, 'rpa/base.html')

class DashboardRpaManager(TemplateView):
    template_name = 'rpa/dashboard_rpamanager.html'
        ...
        for local in localidades:
            ...
            for guarnicao in guarnicoes:
                lista_de_guarnicoes.append({
                    ...
                    'motorista_img': 
                        str(guarnicao.driver.image.url) if(guarnicao.driver and guarnicao.driver.image and guarnicao.driver.image.url != None) else 'sem registro',
                    'piloto_remoto': 
                        str(guarnicao.remote_pilot) if(guarnicao.remote_pilot != None) else 'sem registro',
                    'piloto_remoto_img': 
                        str(guarnicao.remote_pilot.image.url) if(guarnicao.remote_pilot and guarnicao.remote_pilot.image and guarnicao.remote_pilot.image.url != None) else 'sem registro',
                    'piloto_observador': 
                    ...
                    'piloto_observador_img': 
                        ...
                        if(guarnicao.observer_pilot and guarnicao.observer_pilot.image and guarnicao.observer_pilot.image.url != None) else 'sem registro',
                    'local': 
...
class PointsOfInterestView(GroupRequiredMixin, TemplateView):
    ...
    template_name = 'rpa/list_points_of_interests.html'
    group_required = ['profile:rpa_view', 'profile:rpa_basic', 'profile:rpa_advanced']
...
class ChecklistsView(GroupRequiredMixin, TemplateView):
    template_name = 'rpa/list_checklists.html'
...
class RelatoriosView(GroupRequiredMixin, TemplateView):
    template_name = 'rpa/list_reports.html'
    group_required = ['profile:rpa_view', 'profile:rpa_basic','profile:rpa_advanced']
...
class IncidentesView(GroupRequiredMixin, TemplateView):
    template_name = 'rpa/list_incidents.html'
    group_required = ['profile:rpa_basic','profile:rpa_advanced']
...
class AeronavesView(GroupRequiredMixin, TemplateView):
    template_name = 'rpa/list_aircrafts.html'
...
class BateriasView(GroupRequiredMixin, TemplateView):
    template_name = 'rpa/list_batteries.html'
    group_required = ['profile:rpa_advanced']
    limite_de_ciclos = 45
...
class HistoricosPorAeronaveView(GroupRequiredMixin, ListView):
    ...
    template_name = 'rpa/list_aircraft_historic.html'
...
class LegislationsView(ListView):
    model = Legislation
    template_name = 'rpa/list_of_legislation.html'
    context_object_name = 'legislations'
    
    
class TypeOfBatteryView(TemplateView):
    template_name = 'rpa/list_types_of_batteries.html'
